  ...
  if (!salesforceConnection?.accessToken) {
    ...
    try {
      await salesforceConnection.login(
        CONSTANTS.SALESFORCE_EMAIL,
        CONSTANTS.SALESFORCE_PASSWORD + CONSTANTS.SALESFORCE_SECURITY_TOKEN
      );
...
const getLemlistCampaignNameFromCampaignId = async (campaignId: string): Promise<string> => {
  ...
  if (!campaign) {
    ...
    if (
      !lastCampaignCheckedTimestamp ||
      now - lastCampaignCheckedTimestamp > CONSTANTS.LEMLIST_CAMPAIGN_FETCH_COOLDOWN_TIME_IN_MS
    ) {
